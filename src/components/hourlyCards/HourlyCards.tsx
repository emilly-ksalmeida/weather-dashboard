  ...
  return (
    <ScrollArea className="w-full max-w-210 rounded-md border">
      ...
      <div className="flex gap-2 p-4 w-max">
        {Forecast.weather.hourly.hourlyTime.map((day, index) => (
            ...
            <CardContent className="flex flex-col gap-4 justify-center items-center">
              <p className="text-3xl">
                {Forecast.weather.current.weatherCode !== null
                  ? weatherEmojiUnicode[Forecast.weather.hourly.weatherCode[index]]
                  : ""}
              </p>
              <p className="text-3xl">{Forecast.weather.hourly.temperature[index]} °C</p>
              <div className="flex flex-row gap-1 items-center">
                <WiRaindrop />
                <p>{Forecast.weather.hourly.precipitation[index]} mm</p>
              </div>
            </CardContent>
          </Card>
        ))}
